$(document).ready(function(){
    ...
    $('#imagesBlog').change(function(){

        let form_data = new FormData();

        // Read selected files
        let totalFiles = document.getElementById('imagesBlog').files.length;
        for (let index = 0; index < totalFiles; index++) {
            form_data.append("images[]", document.getElementById('imagesBlog').files[index]);
        }
        // AJAX request
        $.ajax({
            url: 'http://127.0.0.1/BookingHotel/a.upload',
            ...
            success: function (response) {
                if (response.status){
                    ...
                    for(let index = 0; index < dataIMG.length; index++) {
                        let src = dataIMG[index];
                        // Add img element in <div id='preview'>
                        $('#previewBlog').append('<img src="'+src+'" width="200px;" height="200px">');
                        $('#inputIMGBlog').append('<input name="images[]" type="hidden" value="'+src+'">');
                    }
                }else {
                    alert(response.data)
                }
